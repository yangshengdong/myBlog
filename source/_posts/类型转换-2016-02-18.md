...
4. 否则，抛出TypeError异常.
1. 如果PreferredType被标志为String，则转换操作的第二步和第三步的顺序会调换.如果没有PreferredType这个参数，则PreferredType的值会按照这样的规则来自动设置：Date类型的对象会被设置为String，其它类型的值会被设置为Number。
...
参考链接：
[阮一峰](http://javascript.ruanyifeng.com/grammar/conversion.html)